...
const NewProduct = () => {
  ...
  const [isLoaded, setIsLoaded] = useState(true); // Set to true initially
  ...
  const fetchproduct = async () => {
    try {
      ...
      if (response.ok) {
        setProducts(postData.data || []); // Update product list
      } else {
        console.error("Failed to fetch products:", postData.message);
      }
    } catch (error) {
      console.error("Error fetching products:", error);
    } finally {
      setIsLoaded(false); // Set loading to false after fetching
    }
  };
