      ...
      if (loginData.email !== "" && loginData.password !== "") {
        ...
        if (res) {
          ...
          const credential = await fetch(
            "api/user/getSingleDataUser",{
              headers: {
                "Authorization": `Bearer ${token}`
              }
        });
          let cred = await credential.json();
          console.log("data from user profile   ",cred.data)
        
          if(cred.data.email.toString()===loginData.email.toString()){
            setisAuth(true);
            localStorage.setItem("loggedin", true);
            localStorage.setItem("loginTime", new Date().getTime());
            localStorage.setItem("userData", JSON.stringify(cred.data));
            setAuthData(cred.data);
          }
          if (loginData.email.toString()==="sharma.harshit295@gmail.com") {
            setLoading(false);
            setinCorrect(false);
            onClose();
